...
client.on('messageReactionAdd', (reaction, user) => {
  if (reaction.message.id == "743143789098434760") {//Geçerli olması istediğiniz mesajın ID'sini yazabilirsiniz.
    if (reaction.emoji.name == "yaniyorsun") {//Dilediğini emojiyi koyabilirsiniz.
      ...
      const yenibiri = new Discord.RichEmbed()
      .setColor("GREEN")
      .setDescription(`${user} adlı kullanıcı sunucuya kayıt oldu.`)
      .setFooter(`© 2020 Astarius Code`)
      client.channels.get("743059017886072912").send(yenibiri)// KAYIT LOGU
}
    }
  });
      /////// OTOROL
...
//// OTOROL SON
    ...
    client.on('ready', ()=>{ client.channels.get('743058976446349355').join() }) // RANDOM PP
...
client.on("message", message => {
  if (message.content === `!js`) {
    const astariuscevap = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setDescription(`${message.author}, <#743059004980199447> kanalından alabilirsin!`)
